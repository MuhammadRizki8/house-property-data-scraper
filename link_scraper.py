...
def extract_links_from_page(url, base_url="https://www.rumah123.com"):
    ...
    Returns:
        list: List of property links found on the page
    """
    try:
        ...
        # Look for cards or containers that might contain property links
        property_cards = soup.select("div.card, div.property-card, div.listing-card, article")
        
        if property_cards:
            logging.info(f"Found {len(property_cards)} property cards on {url}")
            for card in property_cards:
                # Try to find links within the card
                link_elements = card.select("a[href]")
                for link in link_elements:
                    href = link.get("href")
                    if href and href.startswith("/properti/") and href.endswith("/"):
                        full_url = base_url + href
                        if full_url not in property_links:
                            property_links.append(full_url)
        
        # Fallback: if no cards found, look for all links
        if not property_links:
            logging.info(f"No property cards found on {url}, trying generic link extraction")
...
def scrape_all_links(start_url, max_pages=1, min_delay=2, max_delay=5, output_file=None):
    """
...
if __name__ == "__main__":
    ...
    links = scrape_all_links(BASE_URL, max_pages=1)
